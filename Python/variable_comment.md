...
부동소수점 (반올림) 오차 (floating-point rounding error) : 컴퓨터는 2진법을 사용하나, 우리가 쓰는 10진수 소수 중 일부는 2진수로 바꾸면 무한히 반복되는 무한 소수가 됨. 메모리는 유한하기 때문에 컴퓨터는 이 무한 소수를 가장 가까운 근사값으로 잘라서 저장하나, 그 근사치 때문에 미세한 오차가 생기고, 연산 과정에서 문제가 생김.
